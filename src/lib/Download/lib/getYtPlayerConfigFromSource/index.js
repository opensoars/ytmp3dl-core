...
function getYtPlayerConfigFromSource(src, ytplayer_config_re) {
  return new Promise((resolve, reject) => {
    try {
      if (!is.string(src)) return reject('!is.string(src)');
      ...
      // console.log('ytplayer_config_matches', ytplayer_config_matches);
      ...
      if (
        true ||
        (is.array(ytplayer_config_matches) && ytplayer_config_matches[1])
      )
        resolve(JSON.parse(ytplayer_config_matches[1]));
      else reject('!ytplayer_config_matches');
    } catch (err) {
      reject(err);
    }
  });
